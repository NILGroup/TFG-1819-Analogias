...
En Thesaurus Rex las palabras tienen categorías y modificadores. Las categorías son sustantivos que definen a la palabra introducida por el usuario, por ejemplo, la palabra \textit{table} tiene asociadas las categorías \textit{surface, object o item}. Los modificadores son adjetivos que describen a la palabra. Siguiendo con el ejemplo anterior, \textit{table} tiene como modificadores \textit{flat, wooden o inanimate}.

En el caso de introducir dos palabras en la aplicación, por ejemplo \textit{coffee y cola}, la aplicación devuelve las categorías que comparten dichos conceptos, por ejemplo,  \textit{cold-beverage, dark-beverage, stimulating-beverage, etc...}
...
Para el caso de  ``El horizonte es límite de lo que podemos ver", si se establecen como elementos las palabras del texto, sus bigramas son: El horizonte, horizonte es, es el, el límite, límite de, de lo, lo que, que podemos, podemos ver.

En la Figura \ref{fig:metaphormagnet} se puede ver el resultado obtenido en la aplicación para la palabra \textit{house}. En este caso la aplicación devuelve métaforas propias del concepto, por ejemplo \textit{protecting:home}.

Metaphor Magnet también dispone de una API pública que devuelve los resultados en formato XML. En el Listado \ref{lst:xmlmetaphormagnet} se puede ver el resultado devuelto por la API para la palabra \textit{house}. En el XML devuelto aparece la etiqueta \textit{<Source Name>} seguido de la palabra a buscar, en este caso \textit{house}. Otra etiqueta \textit{<Score>} que muestra un número, cuanto mayor sea más acertado será respecto al concepto introducido, por ejemplo \textit{``tall:building"}  tiene una etiqueta \textit{score} de 86. Por último, la etiqueta \textit{<Text>} contiene la metáfora, por ejemplo \textit{``protecting:home"}.
...
\begin{itemize}
	...
	\item Hipónimo: Palabras cuyo significado incluyen el de otra\footnote{https://dle.rae.es/?id=KU5UAn5}. Por ejemplo, gato es hipónimo de mamífero ya que está incluido dentro del conjunto de los mamíferos.
...
En la Figura \ref{fig:eurowordnet2} aparece la información devuelta por EuroWordNet para la palabra ``casa''. Los datos obtenidos son:
\begin{itemize}
	...
	\item Sinónimos: como por ejemplo, edificio o inmueble.
	\item Definición: para contextualizar el concepto. Por ejemplo, una estructura que tiene un techo, paredes y se encuentra más o menos permanente en un solo lugar.
	\item Ejemplo: en el caso de que la definición no sea suficientemente clara para el usuario, ayude al usuario a comprender el concepto. Por ejemplo, había un edificio de tres pisos en la esquina. 
\end{itemize} 

Además, en el desplegable de la izquierda se muestra la información que se puede consultar de dicho concepto. Por ejemplo, si seleccionamos \textit{has hyperonym} se obtendrán los hiperónimos de casa, o seleccionando \textit{has hyponym} se obtendrán los hipónimos.
