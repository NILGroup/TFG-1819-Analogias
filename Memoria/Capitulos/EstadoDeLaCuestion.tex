...
Se puede ver en el Listing \ref{lst:ejemWordNet} la forma de obtener la lista de \textit{synsets} de la palabra coche. Primero se debe importar el paquete nltk y el \textit{corpus} WordNet. A continuación, se invoca al método \textit{synset} de WordNet, introduciendo como parámetros la palabra de la que se quiere obtener los \textit{synsets} y el lenguaje en el que está dicha palabra (si es una palabra del lenguaje inglés se puede dejar el segundo parámetro en blanco). Después, del primer \textit{synset} de la lista, se muestran por pantalla cada uno de sus sinónimos (en español), su definición, una oración de ejemplo, sus hiperónimos, hipónimos y holónimos. Por último, del primer \textit{lemma}, se obtienen sus antónimos.

	\lstset{language=Python}
\begin{lstlisting}[caption= Ejemplo de relaciones entre synset, label={lst:ejemSynset}, frame=single]
import nltk
from nltk.corpus import wordnet as wn
